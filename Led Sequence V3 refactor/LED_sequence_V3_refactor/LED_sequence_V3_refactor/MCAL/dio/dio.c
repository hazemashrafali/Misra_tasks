...
enu_error_state_t DIO_portMode	(uint8 port,enu_port_direction_t enu_port_direction)
{
	enu_error_state_t enu_error_state = VALID_OPERATION;
	if((port >= PORTA_ID) && (port < MAX_PORT_ID))
	{
...
enu_error_state_t DIO_readPort	(uint8 port,uint8** pptr_value)
{
	enu_error_state_t enu_error_state = VALID_OPERATION;
	if((port >= PORTA_ID) && (port < MAX_PORT_ID))
	{
